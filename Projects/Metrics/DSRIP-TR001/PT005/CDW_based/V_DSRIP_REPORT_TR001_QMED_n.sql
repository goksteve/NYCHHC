    ...
    (
     ...
     UNION
      ...
      JOIN cdw.dim_hc_departments dp
        ON dp.network = vl.network AND dp.location_id = vl.location_id AND dp.service_type = 'BH'
    ) q
  ),
  visit_providers AS
  (
    ...
    (
      SELECT
        network, visit_id, attending_emp_provider_id AS provider_id
      FROM visits
...
(
  SELECT
    ...
    q.streetadr, q.apt_suite, q.city, q.state, q.zipcode, q.country, q.home_phone, q.day_phone,
    q.mrn,
    q.visit_id,
    q.visit_number,
    ...
    prv.provider_name||' - '||prv.physician_service_name bh_provider_info,
    ROW_NUMBER() OVER
    (
      PARTITION BY q.network, q.visit_id
      ORDER BY
      CASE
        WHEN UPPER(prv.physician_service_name) LIKE '%PSYCH%' THEN 1
        WHEN prv.physician_service_name IS NOT NULL THEN 2
        ELSE 3 
      END,
      provider_name
    ) provider_rnum,  
    ...
    (
      PARTITION BY q.network, q.visit_id
      ORDER BY CASE WHEN pyr.payer_group = 'Medicaid' THEN 1 ELSE 2 END,
      vsp.payer_number
    ) payer_rnum 
  FROM
  (
    SELECT
      report_period_start_dt, network, facility_id, facility_name, 
      patient_id, mrn, patient_name,
      patient_dob, prim_care_provider,
      streetadr, apt_suite, city, state, zipcode, country, home_phone, day_phone,
    ...
    ( 
      SELECT
        ...
        CASE
          ...
          ELSE pat.name
        END patient_name,
        NVL(TO_DATE(mdm.dob, 'YYYY-MM-DD'), pat.birthdate) patient_dob,
        NVL(vst.mrn, mdm.mrn) mrn,
        mdm.streetadr, mdm.apt_suite, mdm.city, mdm.state, mdm.zipcode, mdm.country, mdm.home_phone, mdm.day_phone,
        NVL(TO_CHAR(mdm.eid), vst.network||'_'||vst.patient_id) patient_gid,
        ...
        CASE WHEN vst.visit_type_cd = 'IP' THEN vst.admission_dt END AS re_admission_dt,
        CASE WHEN vst.visit_type_cd <> 'IP' THEN vst.visit_id END AS follow_up_visit_id, 
        CASE WHEN vst.visit_type_cd <> 'IP' THEN vst.visit_number END AS follow_up_visit_number, 
        CASE WHEN vst.visit_type_cd <> 'IP' THEN vst.admission_dt END AS follow_up_dt, 
        CASE WHEN vst.visit_type_cd <> 'IP' THEN fd.facility_name END AS follow_up_facility,
        CASE WHEN vst.visit_type_cd <> 'IP' THEN fc.name END AS follow_up_fin_class,
        ROW_NUMBER() OVER
        (
          PARTITION BY vst.network, vst.visit_id
          ORDER BY
          CASE
            WHEN SUBSTR(mdm.facility_name, 1, 2) = fd.facility_cd AND mdm.mrn = vst.mrn THEN 1
            WHEN SUBSTR(mdm.facility_name, 1, 2) = fd.facility_cd THEN 2
            ELSE 3
          END, eid
        ) mdm_rnum
      FROM visits vst
      ...
      JOIN cdw.dim_hc_facilities fd
        ON fd.network = vst.network AND fd.facility_id = vst.facility_id
  ...
  LEFT JOIN cdw.visit_segment_payer vsp
    ON vsp.network = q.network AND vsp.visit_id = NVL(q.follow_up_visit_id, q.visit_id)
  LEFT JOIN cdw.dim_payers pyr
    ON pyr.network = vsp.network AND pyr.payer_id = vsp.payer_id
  LEFT JOIN visit_providers vpr
    ON vpr.network = q.network AND vpr.visit_id = NVL(q.follow_up_visit_id, q.visit_id)
