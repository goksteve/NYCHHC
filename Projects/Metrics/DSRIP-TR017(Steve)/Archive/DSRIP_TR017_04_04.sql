...
(
    SELECT --+ materialize
     ...
     COUNT(DISTINCT r.test_type) OVER (PARTITION BY r.patient_id) pat_rslt_cnt
     FROM  fact_visits v
     JOIN a1c_ldl r   ON r.network = v.network AND r.visit_id = v.visit_id
     LEFT JOIN tmp_pcp_bh lst ON  r.network = lst.network AND r.patient_id = lst.patient_id and  r.visit_id  = lst.visit_id
