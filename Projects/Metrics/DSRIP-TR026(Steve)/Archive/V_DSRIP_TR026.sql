CREATE OR REPLACE VIEW V_DSRIP_TR026 AS
...
AS
 (
    ...
    ELSE ROUND(days_covered / (report_dt - tp.order_dt), 1) * 100
    END
    AS pds_ratio,
    CASE WHEN ROUND(days_covered / (report_dt - tp.order_dt), 1) * 100 < 80 THEN 1 ELSE 0 END less_80_flag,
    dv.visit_id,
    dv.diagnosis_dt,
    dv.icd_code,
    dv.problem_comments,
...
WHERE 1=1
 AND FLOOR((v.admission_dt - pp.birthdate) / 365) BETWEEN 18 AND 64
