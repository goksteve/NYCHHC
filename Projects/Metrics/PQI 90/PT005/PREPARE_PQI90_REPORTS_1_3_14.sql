CREATE OR REPLACE PROCEDURE prepare_pqi90_reports_1_3_14(p_report_month IN DATE DEFAULT NULL) AS
/*
  2018-04-03, GK: created 
*/
  d_report_mon  DATE;
  n_cnt         PLS_INTEGER := 0;
BEGIN
  xl.open_log('PREPARE_PQI90_REPORTS_1_3_14', SYS_CONTEXT('USERENV','OS_USER')||': Generating PQI90 reports #1, #3 and #14', TRUE);
  ...
  DELETE FROM dsrip_report_pqi90_1_3_14 WHERE report_period_start_dt = d_report_mon;
  n_cnt := SQL%ROWCOUNT;
  
  DELETE FROM dsrip_report_results
  WHERE report_cd = 'PQI90-1_3_14' AND period_start_dt = d_report_mon;
  ...
  (
    ...
    p_tgt => 'DSRIP_REPORT_PQI90_1_3_14',
    p_src => 'V_DSRIP_REPORT_PQI90_1_3_14',
    p_commit_at => -1
  );
  /*
  etl.add_data
  (
    ...
    p_src => 'SELECT 
      ...
      FROM dsrip_report_pqi90_1_3_14 r
