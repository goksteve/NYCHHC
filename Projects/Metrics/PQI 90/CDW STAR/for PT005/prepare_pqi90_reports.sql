CREATE OR REPLACE PROCEDURE prepare_pqi90_reports(p_report_month IN DATE DEFAULT NULL) AS
/*
...
BEGIN
  ...
  DELETE FROM dsrip_report_pqi90 WHERE report_dt = d_report_mon;
  n_cnt := SQL%ROWCOUNT;
  ...
  (
    ...
    p_tgt => 'DSRIP_REPORT_PQI90',
    p_src => 'V_DSRIP_REPORT_PQI90',
    p_commit_at => -1
  );
  ...
  (
    ...
    p_src => 'SELECT 
      ...
      WHERE r.report_dt = '''||d_report_mon||'''
      GROUP BY GROUPING SETS((report_dt, network, Facility),(report_dt))',
    p_commit_at => -1
  );
