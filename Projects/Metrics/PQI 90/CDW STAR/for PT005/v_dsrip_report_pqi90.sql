...
WITH
  ...
  (
    ...
    LEFT JOIN /*cdw.hhc_patient_dimension p*/ cdw.dim_patients p
      ON p.network = v.network AND p.patient_id = v.patient_id AND p.current_flag = 1
    LEFT JOIN /*pt005.facility_dimension fd*/ cdw.dim_hc_facilities fd
      ON fd.facility_key = v.facility_key 
    LEFT JOIN cdw.dim_providers prv1
      ON prv1.provider_key = v.admitting_provider_key
    LEFT JOIN cdw.dim_providers prv2
