CREATE OR REPLACE VIEW v_dsrip_report_tr044
AS
WITH report_dates 
...
--    AND pd.patient_id = 318397 AND pd.network = 'GP1'
    JOIN cdw.dim_patients pp ON pp.network  = pd.network AND pp.patient_id  = pd.patient_id AND current_flag  = 1 AND LOWER(sex) IN ('male', 'female')  
...
--  * from pat_combo;
  ...
  (
    ...
    LEFT JOIN cdw.fact_visit_payers fct_pyrs
      ON fct_pyrs.visit_key = vst.visit_key
    LEFT JOIN cdw.dim_payers pyr1
      ON pyr1.payer_key = fct_pyrs.first_payer_key
    LEFT JOIN cdw.dim_payers pyr2
      ON pyr2.payer_key = fct_pyrs.first_payer_key 
    LEFT JOIN cdw.dim_payers pyr3
...
SELECT  
  ...
  CASE
    WHEN pcp.pcp_visit_id IS NULL AND cardio.cardio_visit_id IS NULL
    THEN 1
    ELSE 0
  END AS non_pcp_flag 
...
LEFT JOIN cdw.dim_providers prvdr1
ON prvdr1.provider_key = pcp_vst_provider_key
LEFT JOIN cdw.dim_providers prvdr2
