...
WITH report_dates AS
 ...
 pat_diag AS
      (
        SELECT --+  materialize 
        pd.network, pd.patient_id, pp.sex , age, pd.onset_date as dig_dt, 
        ...
        WHERE  mc.criterion_id IN (70,71,73,85,84,83)  AND d.status_id IN (0, 6, 7,8)
        AND   FLOOR((d.report_year - pp.birthdate) / 365) BETWEEN  ( CASE WHEN LOWER(sex) = 'male' THEN  21 ELSE 40 END ) AND 75 
             
      )

select /*+ PARALLEL(32) */  * from  pat_diag
...
sel_pat_diag
  ...
  (
     SELECT network, patient_id FROM pat_diag
      WHERE   crit_id in(70,71) 
    MINUS
  (
    SELECT DISTINCT network, patient_id
    FROM pat_diag  WHERE crit_id in(73,85,84,83) 
UNION  
