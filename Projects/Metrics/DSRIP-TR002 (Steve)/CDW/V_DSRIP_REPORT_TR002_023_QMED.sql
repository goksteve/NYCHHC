DROP VIEW v_dsrip_report_tr002_023_qmed;

CREATE OR REPLACE FORCE VIEW v_dsrip_report_tr002_023_qmed
(
 network,
 facility_name,
 mrn,
...
(
  ...
  ADD_MONTHS(
  NVL(TO_DATE(SYS_CONTEXT('USERENV', 'CLIENT_IDENTIFIER')), TRUNC(SYSDATE, 'MONTH')),-24)
  start_dt
  FROM DUAL
),
 diab_diagnoses AS
...
WHERE
v.admission_dt >= rd.start_dt AND v.admission_dt < rd.report_dt
)
SELECT --+ parallel(32)
pp.network,
f.facility_name,
...
FROM
pat_list pp
JOIN diab_diagnoses dd ON dd.patient_key = pp.patient_key AND dd.rnum = 1
JOIN dim_hc_facilities f ON f.facility_key = pp.facility_key
LEFT JOIN fact_visit_metric_results r ON r.visit_id = pp.visit_id AND r.a1c_final_orig_value IS NOT NULL
WHERE
pp.visit_rnum = 1)
WHERE
result_rnum = 1;