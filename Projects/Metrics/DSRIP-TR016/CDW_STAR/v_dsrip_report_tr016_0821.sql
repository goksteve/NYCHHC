CREATE OR REPLACE FORCE VIEW V_DSRIP_REPORT_TR016
(
 ...
 WITH -- 10-Apr-2018, GK: Converting into CDW Star schema
 ...
 report_dates AS
  (SELECT --+ materialize
    ...
    ADD_MONTHS(NVL(TO_DATE(SYS_CONTEXT('USERENV', 'CLIENT_IDENTIFIER')), TRUNC(SYSDATE, 'MONTH')), -12)
     year_back_dt
   FROM
    DUAL),
 prescriptions AS
  ...
  --  pr.mrn,
    ...
    ROW_NUMBER()
    OVER(
     PARTITION BY NVL(TO_CHAR(mdm.eid), pr.network || '-' || pr.patient_id),
                  NVL(dnm.drug_type_id, dscr.drug_type_id)
     ORDER BY pr.order_dt DESC)
     rnum
   FROM
    report_dates rd
    JOIN fact_patient_prescriptions pr ON pr.order_dt <= rd.year_back_dt AND pr.network NOT IN ('QHN', 'SBN') -- exclude Networks that have switched to EPIC
   ...
   WHERE
    dnm.drug_type_id IN (33, 34) OR dscr.drug_type_id IN (33, 34) -- Diabetes and Antipsychotic Medications
                                                                 ),
 diagnoses AS
  (SELECT --+ materialize
   ...
   WHERE
    pd.network NOT IN ('QHN', 'SBN') AND pd.diag_coding_scheme IN ('ICD-10', 'ICD-9') --AND pd.current_flag = '1'
                                                                                     AND pd.end_date IS NULL),
 rslt_meta_data AS
  (SELECT --+ materialize
    dt.report_dt, dt.year_back_dt, mc.*
   FROM
    report_dates dt CROSS JOIN meta_conditions mc
   WHERE
    mc.criterion_id IN (4, 23)),
 ...
 diabetes_prescriptions AS
  (SELECT --+ materialize
   ...
   FROM
    prescriptions
   WHERE
    drug_type_id = 33 -- Diabetes Prescriptions
   GROUP BY
    patient_gid),
 a1c_glucose_tests AS
  (SELECT --+ materialize
    NVL(TO_CHAR(mdm.eid), a.network || '-' || a.patient_id) patient_gid,
    a.network,
    facility_key,
    patient_id,
    visit_id,
    ...
    result_value,
    ROW_NUMBER()
     OVER(PARTITION BY NVL(TO_CHAR(mdm.eid), a.network || '-' || a.patient_id) ORDER BY a.result_dt DESC)
     rnum
   FROM
    (SELECT -- ordered use_hash(r v)
      ...
      r.data_element_id,
      r.result_value,
      ROW_NUMBER() OVER(PARTITION BY v.network, v.patient_id ORDER BY r.event_id DESC, r.data_element_id)
       evnt_rnum
     FROM
      rslt_meta_data mc
      JOIN fact_results r
      ...
      LEFT JOIN ref_visit_types vt ON vt.visit_type_id = v.final_visit_type_id) a
    LEFT JOIN dconv.mdm_qcpr_pt_02122016 mdm
 ...
 SELECT --+ parallel(32) /*USE_HASH(f pd pr)*/
  ...
  CASE WHEN psych.coding_scheme IS NOT NULL THEN psych.coding_scheme || ': ' || psych.diag_code END
   bh_diag_code,
  psych.diag_description bh_diagnosis,
  ROW_NUMBER()
  OVER(
   PARTITION BY amed.patient_gid, tst.network, tst.visit_id
   ORDER BY CASE WHEN pr.payer_group = 'Medicaid' THEN 1 ELSE 2 END)
   rnum
 FROM
  prescriptions amed
  ...
  LEFT JOIN dim_hc_facilities f --dim_hc_facilities
   ON f.facility_ID = amed.facility_ID AND amed.network  = f.network
...
GRANT SELECT ON V_DSRIP_REPORT_TR016 TO PUBLIC;
