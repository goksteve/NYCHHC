CREATE OR REPLACE FORCE VIEW V_TST_GK_TR010_NEW_RX
(
 report_dt,
 begin_dt,
 end_dt,
 ...
 icd_code,
 problem_description,
 asthma_cntrlr_med_name,
 asthma_cntrlr_med_disp_date,
 asthma_other_med_disp_date,
 asthma_other_med_name,
 asthma_cntrlr_med_cnt,
 asthma_other_med_cnt,
 med_luk_antbdy_dspns_evnt_cnt,
 cnt_of_total_medications,
 ratio,
 numerator_flag,
 clinic_visit_count,
 ...
 WITH -- 20-JUN-2018, GK: script using star schema tables.
     dt AS
       (SELECT --+ materialize
         ...
         CASE
          WHEN TO_CHAR(
           ...
           TRUNC(
            ...
            'YEAR')
          ELSE
           TRUNC(NVL(TO_DATE(SYS_CONTEXT('USERENV', 'CLIENT_IDENTIFIER')), TRUNC(SYSDATE, 'MONTH')), 'YEAR')
         END
          AS msrmnt_yr,
       ...
       (SELECT --+ materialize
        ...
        FROM
         (SELECT
          ...
          FROM
           dt
           JOIN cdw.fact_visits /*ud_master.visit*/
                               vst ON vst.admission_dt BETWEEN dt.begin_dt AND dt.report_dt --AND vst.patient_id = 1572634 AND vst.network = 'CBN'
           --        AND vst.network = 'NBX' and vst.PATIENT_ID = 57963
           JOIN cdw.fact_visit_diagnoses /*goreliks1.visit_event_icd_code*/
           ...
           LEFT JOIN cdw.ref_visit_types vt1 ON vt1.visit_type_id = vst.final_visit_type_id
            ...
            ON vt2.visit_type_id = vst.initial_visit_type_id
               AND NOT EXISTS
                    (SELECT
                     ...
                     FROM
                      cdw.fact_patient_diagnoses cmv
                      JOIN meta_conditions metac1 ON metac1.VALUE = cmv.diag_code
                     WHERE
                      metac1.criterion_id = 21
                      AND metac1.include_exclude_ind = 'E'
                      AND metac.network = 'ALL'
                      AND cmv.patient_id = vst_prb.patient_id
                      AND cmv.network = vst_prb.network)) a) --  select --+ parallel(32)
      ...
      pcp_vst_dt AS
       (SELECT --+ materialize
        ...
        FROM
         ...
         JOIN cdw.fact_visits vst ON vst.patient_id = vst_lkp.patient_id AND vst.network = vst_lkp.network --AND vst.patient_id = 1572634 AND vst.network = 'CBN'
         --    AND vst.network = 'NBX' and vst.PATIENT_ID = 57963
         JOIN cdw.dim_hc_departments pcp
          ON pcp.department_key = vst.last_department_key AND pcp.service_type = 'PCP') --  select --+ parallel(32)
                                                                                        --  * from pcp_vst_dt;
      ,
         ...
         ((
          ...
          PIVOT
           ...
           IN ('Clinic' clinic_visit_count,
              ...
              'Outpatient' outpatient_visit_count)))) --  select --+ paralle(32)
                                                      --  * from ptnt_vst_type_cnt;
      ,
       ...
       (SELECT --+ materialize
        ...
        FROM
         (SELECT
          ...
          FROM
           dt
           JOIN cdw.fact_patient_prescriptions rx ON rx.order_dt BETWEEN dt.begin_dt AND dt.report_dt --AND rx.patient_id = 1572634 AND rx.network = 'CBN'
           --        AND rx.network = 'NBX' and rx.PATIENT_ID = 57963
           JOIN cdw.ref_drug_descriptions rd
            ON rx.drug_description = rd.drug_description AND rd.drug_type_id IN (40, 41))) --  SELECT --+ parallel(32)
        ...
        FROM
         (SELECT
          ...
          FROM
           ...
           --        AND rx.network = 'NBX' and rx.PATIENT_ID = 57963
           JOIN cdw.ref_drug_descriptions rd
            ON rx.drug_description = rd.drug_description AND rd.drug_type_id = 42)) --  select --+ parallel(32)
                                                                                    --  * from asthma_cntrlr_med_dspns_evnt;
      ,
 ...
 --    select --+ parallel(32)
 --  * from asthma_other_med_dspns_evnt;
 ...
 SELECT --+ parallel(32)
  ...
  CASE
   WHEN (e.asthma_cntrlr_med_cnt / (e.asthma_cntrlr_med_cnt + d.asthma_other_med_cnt)) >= 0.50 THEN 'Y'
   ELSE 'N'
  END
   numerator_flag,
 ...
 FROM
  ...
  JOIN cdw.dim_patients ptnt
   ON ptnt.patient_id = a.patient_id
      ...
      --         AND ptnt.network = 'NBX' and ptnt.PATIENT_ID = 57963

      AND (FLOOR((ADD_MONTHS(TRUNC(SYSDATE, 'year'), 12) - 1 - ptnt.birthdate) / 365) BETWEEN 5 AND 64)
  LEFT JOIN cdw.dim_payers pm ON pm.payer_id = a.first_payer_key AND pm.network = a.network
