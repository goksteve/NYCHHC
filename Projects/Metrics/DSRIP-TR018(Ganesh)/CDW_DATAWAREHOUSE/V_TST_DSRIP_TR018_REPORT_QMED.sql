CREATE OR REPLACE VIEW v_tst_dsrip_tr018_report_qmed
AS
WITH 
  dt AS 
  (
...
--      TRUNC (ADD_MONTHS(TRUNC(SYSDATE, 'MONTH'), -1), 'MONTH') end_dt
      ...
      TRUNC ( TO_DATE (SYS_CONTEXT ('USERENV', 'CLIENT_IDENTIFIER')), 'MONTH')   AS end_dt
    FROM DUAL
  ),
  htn_ptnt_lkp AS 
  (
    ...
    JOIN fact_patient_diagnoses fpd on  fpd.onset_date >= dt.msrmnt_yr_start_dt --AND fpd.network = 'CBN'
     AND fpd.onset_date <  
          CASE
             ...
             THEN report_period_start_dt
             ELSE ADD_MONTHS(msrmnt_yr_start_dt, 6)
           END
  ...
  (
    SELECT --+ materialize 
      dt.report_period_start_dt, dt.begin_dt /* report_period_start_dt */, dt.end_dt /* report_period_end_dt */, v.network, v.visit_id,v.final_visit_type_id,
  ...
  (
    SELECT -- use_hash(r evnt v)
      v.report_period_start_dt, v.begin_dt /* report_period_start_dt */, v.end_dt /* report_period_end_dt */, v.network, v.facility_key, v.patient_id,
...
--     AND r.network = 'CBN'
	...
	(
		SELECT 
      g.report_period_start_dt, g.begin_dt /* report_period_start_dt */, g.end_dt /* report_period_end_dt */, g.network, g.facility_key, g.patient_id,
    ...
    (  
      SELECT   
        ...
        begin_dt /* report_period_start_dt */, 
        end_dt /* report_period_end_dt */, 
        network, 
        facility_key, 
        patient_id,
        visit_id, 
        ...
        END flag_150_90          
      FROM rslt
      GROUP BY report_period_start_dt, begin_dt, end_dt, network, facility_key, patient_id, visit_id, final_visit_type_id, 
