CREATE OR REPLACE VIEW V_STG_VISIT_METRICS_EPIC 
AS
 WITH 
  get_dates
  AS
...
SELECT --+ materiaalize
   ...
   FROM (
         ...
         CAST( NVL(order_proc.order_time, pat_enc.contact_date) AS DATE) AS tabacco_result_dt   ,
          ...
          AND order_proc.result_time >= '01-APR-2016' OR (pat_enc.enc_type_c IN ('109'))
        ) where rn = 1
     UNION 
        SELECT --+ materialize
        x.network,
  ...
  (
    ...
    FROM
        ...
        SELECT
          ...
          LEFT JOIN  tmp_tabacco_screen tt  ON d.patient_id = tt.patient_id AND d.visit_id = tt.visit_id
