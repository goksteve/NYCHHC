  ...
  WHERE
   ...
   diag_pat AS
    (
     ...
     WHERE
      v.epic_flag = 'Y' AND v.admission_date_time >= LAST_DAY(ADD_MONTHS(SYSDATE, -1))
   ),
    pat_inc_exc AS
    (
...
(
  SELECT
   ...
   FROM
      ...
      AND order_proc.result_time >= '01-APR-2016'
      ) l
  WHERE
  l.rn = 1
 ),
     ...
     (
      ...
      FROM
       (
        ...
        WHERE
         meas.recorded_time >= '01-APR-2016'
       ) measflow
      WHERE
       measflow.recorded_time =
        (
         SELECT
          MAX(maxflow.recorded_time)
         FROM
          (SELECT /*+ PARALLEL (32) */
            rec.inpatient_data_id,
            meas.flo_meas_id,
            ...
            meas.meas_value
           FROM
            epic_clarity.ip_flwsht_rec rec
            INNER JOIN epic_clarity.ip_flwsht_meas meas ON rec.fsd_id = meas.fsd_id
           WHERE
            meas.recorded_time >= '01-APR-2016') maxflow
         WHERE
          measflow.inpatient_data_id = maxflow.inpatient_data_id AND measflow.flo_meas_id = maxflow.flo_meas_id
        )
       ) measmax1
      WHERE
      NOT REGEXP_LIKE(measmax1.meas_value, '[^0-9, /]+')
    ) bp
    ON pe.inpatient_data_id = bp.inpatient_data_id AND bp.flo_meas_id = '5'
    WHERE
   (pe.contact_date >= '01-APR-2016') AND bp.bp_diastolic IS NOT NULL AND bp.bp_systolic IS NOT NULL
   ),
a1c AS
(
 SELECT /*+ PARALLEL (32) */
...
SELECT /*+ Parallel (32) */
 ...
 sex,
 age AS patient_age_at_admission,
 coding_scheme,
 diagnosis_name,
 icd_code,
 is_primary_problem,
  CASE WHEN asthma_ind > 0 THEN 1 END AS asthma_ind ,
  CASE WHEN bh_ind > 0 THEN 1 END AS bh_ind,
  CASE WHEN breast_cancer_ind > 0 THEN 1 END AS breast_cancer_ind,
  CASE WHEN diabetes_ind > 0 THEN 1 END AS diabetes_ind,
  CASE WHEN heart_failure_ind > 0 THEN 1 END AS heart_failure_ind,
  CASE WHEN hypertension_ind > 0 THEN 1 END AS hypertension_ind,
  CASE WHEN kidney_diseases_ind > 0 THEN 1 END AS kidney_diseases_ind,
  CASE WHEN pregnancy_ind > 0 THEN 1 END AS pregnancy_ind,
 pregnancy_onset_dt,
 CASE WHEN nephropathy_screen_ind > 0 THEN 1 END AS  nephropathy_screen_ind,
 CASE WHEN retinal_eye_exam_ind  > 0 THEN 1 END AS retinal_eye_exam_ind,
 ldl_order_time,
 ldl_result_time,
 ldl_calc_value,
 bp_diastolic,
