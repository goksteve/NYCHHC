CREATE OR REPLACE VIEW v_fact_patient_prescriptions_d
 AS
...
WITH rx AS
   (
    SELECT --+ materialize
       ...
       FROM
        dim_patients pp
        JOIN  prescription rx  on rx.network = pp.network and rx.patient_id  = pp.patient_id and pp.current_flag  = 1
  ...
  WHERE
    ...
    SELECT /*+ PARALLEl (32) */
     ...
     rx.order_span_id,
     rx.order_dt,
     rx.drug_description AS orig_drug_description,
     rx.dosage,
     rx.frequency,
     rx.rx_quantity,
     rx.rx_refills,
     ...
     CASE WHEN rx.misc_name IS NULL 
      THEN  NVL(LOWER(dn.derived_product_name),'n/a')
     ELSE  NVL(LOWER(rx.derived_product_name),'n/a') END AS drug_description,
     'QCPR' AS source,
...
UNION
      SELECT
       ...
       NVL(LOWER(v.proc_name), 'n/a') AS drug_name,
       '-99' AS rx_allow_subs,
       ...
       NVL(LOWER(NVL(g.name, a.display_name)),'n/a')  AS drug_description,
       'EPIC' AS source,
       TRUNC(SYSDATE) AS load_dt
      FROM
...
UNION 
      ...
      'n/a' AS misc_name,
       ...
       NVL(LOWER( pm.dkv_drug_name), 'n/a') AS drug_name,
       ...
       NVL(LOWER( NVL( pm.dkv_dnid_name,NVL(pm.dkv_generic_dnid_name ,pm.dkv_product_string))),'n/a') AS drug_description,
       'NQCPR' AS source,
       TRUNC(SYSDATE) AS load_dt
      FROM
       dim_patients pp
