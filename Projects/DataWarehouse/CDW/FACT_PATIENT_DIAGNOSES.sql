...
CREATE INDEX idx_fact_diags_pat_key ON fact_patient_diagnoses(network, patient_key) LOCAL PARALLEL 32;
ALTER INDEX idx_fact_diags_pat_key NOPARALLEL;

CREATE INDEX idx_fact_diags_pat_id ON fact_patient_diagnoses(network, patient_id) LOCAL PARALLEL 32;
ALTER INDEX idx_fact_diags_pat_id NOPARALLEL;

CREATE UNIQUE INDEX pk_fact_patient_diags ON fact_patient_diagnoses(patient_id, problem_number, diag_coding_scheme, diag_code, network) LOCAL PARALLEL 32;
ALTER INDEX pk_fact_patient_diags NOPARALLEL;

ALTER TABLE fact_patient_diagnoses ADD CONSTRAINT pk_fact_patient_diags PRIMARY KEY(patient_id, problem_number, diag_coding_scheme, diag_code, network)
