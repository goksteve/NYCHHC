...
SELECT 
  ...
  (
    ...
    (
      ...
      ('2' AS emp,'4' AS license,'27' AS social_security,'29' AS sdg_emp_no,'36' AS prac_npi,'39' AS npi,'40' AS license_exp_date)))
      SELECT 
        DISTINCT fi.network,
        fi.provider_id,
        fi.provider_name,
      ...
      (
        ...
        LEFT JOIN medical_specialty ms ON efms.physician_service_id = ms.physician_service_id AND efms.facility_id = ms.facility_id AND efms.network = ms.network) fi
        WHERE rnk = 1
  ) p
