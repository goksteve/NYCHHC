CREATE TABLE STEVE_PAT_ARCHIVE
nologging
compress basic
parallel 32
AS
	...
	WITH patient_archive_stage AS
			  (SELECT network,
				  ...
				  FROM (SELECT network,
									...
									archive_source_id,
									ROW_NUMBER()
										OVER(PARTITION BY network, patient_id, TRUNC(archive_time, 'HH12')
											  ORDER BY
												  network,
												  patient_id ASC,
												  archive_number DESC,
							...
							WHERE 	 1 = 1
									...
									AND EXISTS
											 (SELECT 1
												 FROM patient pp
												WHERE pp.patient_id = pa.patient_id AND pp.network = pa.network)) pat
				 WHERE row_num = 1)
	SELECT /*+ PARALLEL(32) */
			network,
			 ...
			 NVL( employer_name, 'N/A') employer_name,
	  ...
	  FROM (
			  ...
			  SELECT /*+ PARALLEL (32) */
					  p.network,
						...
						p.date_of_death,
						p.apt_suite,
						NVL(p.street_address, 'NA') street_address,
