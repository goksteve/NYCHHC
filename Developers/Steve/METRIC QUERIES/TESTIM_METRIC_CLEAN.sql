...
WITH crit_metric AS
     ...
     tmp AS
      ...
      AND r.event_status_id IN (6, 11)   AND r.network = 'GP1'
      WHERE
...
AND NOT REGEXP_LIKE(TRIM(LOWER(r.result_value)), '(^3n)|(^x\[p)|(..)')
    )
 SELECT --+ PARALLEL (48)
 q.network,
 q.visit_id,
 ...
 CASE
      ...
      FN_STR_TO_NUMBER
     (
      q.network,  q.result_value,
       SUBSTR(TRIM(REGEXP_REPLACE(REGEXP_REPLACE(TRIM(REGEXP_REPLACE(REGEXP_REPLACE(q.result_value, 
        '(([[:digit:]]{1,2})-([[:alpha:]]{3,9})-([[:digit:]]{2,4}))|(([[:digit:]]{1,2}) ([[:alpha:]]{3,9}) ([[:digit:]]{2,4}))|(([[:digit:]]{1,2})\/([[:digit:]]{1,2})\/([[:digit:]]{2,4}))| ([[:digit:]]{4,10})|(HgbA1C)'),
       '([-?!.\=/+,?!><$*#^@%)(&]+$)|(^[-?!.0\=/+,?!><$*#@%)(0&]+)|([[:alpha:]-)(#?!$%])')),
     '^[-?!.0\=/+,?!><$*#@%]'),
    '([ ,:*&%$;=>/`])|(\.+$)')), 1, 3)
      )
