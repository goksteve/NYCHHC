...
WITH crit_metric AS
     ...
     tmp AS
      ...
      AND r.event_status_id IN (6, 11)   AND r.network = 'GP2'
      WHERE
      ...
      '(([[:digit:]]{1,2})-([[:alpha:]]{2,9})-([[:digit:]]{2,4}))|(([[:digit:]]{1,2}) ([[:alpha:]]{2,9}) ([[:digit:]]{2,4}))|(([[:digit:]]{1,2})\/([[:digit:]]{1,2})\/([[:digit:]]{2,4}))|
       (([[:alpha:]]{2,9}) ([[:digit:]]{2,4}),([[:digit:]]{2,4}))|([[:digit:]]{4,10})|([[:alpha:]-\=/.+ ,?!><$*#^@%)(0&])|(([[:digit:]]{1,2})([[:alpha:]]{3,9})([[:digit:]]{2,4}))'
      )  IS NOT NULL
      AND REGEXP_REPLACE(SUBSTR(TRIM(r.result_value), 1, 1), '[-\=/.+ ,?!><$*#^@%)(0&]' ) IS NOT NULL  
      AND SUBSTR(TRIM(REGEXP_REPLACE(REGEXP_REPLACE
       (r.result_value,
      '([-?!.+_]+$)|([[:alpha:]]\.)|([\)(])|(([[:digit:]]{1,2})-([[:alpha:]]{2,9})-([[:digit:]]{2,4}))|(([[:digit:]]{1,2}) ([[:alpha:]]{2,9}) ([[:digit:]]{2,4}))|
       (([[:digit:]]{1,2})\/([[:digit:]]{1,2})\/([[:digit:]]{2,4}))|(([[:alpha:]]{2,9}) ([[:digit:]]{2,4}),([[:digit:]]{2,4}))'), '([[:digit:]]{4,10})|([^[:digit:]. ])|(([[:digit:]]{1,2})([[:alpha:]]{3,9})([[:digit:]]{2,4}))')), 1, 1
       ) <> '0'
...
AND NOT REGEXP_LIKE(TRIM(LOWER(r.result_value)), '(^3n)|(^x\[p)|([..])|(^kct)|(over)|(a1c)' )
    )
 SELECT --+ PARALLEL (48)
 q.network,
 q.visit_id,
 ...
 CASE
      WHEN q.criterion_id IN (10,23) THEN -- Glucose / LDL  <= 1000
        ...
        SUBSTR(
        TRIM(
        REGEXP_REPLACE(REGEXP_REPLACE(TRIM(REGEXP_REPLACE(REGEXP_REPLACE(' Other no recent HgBA1C_ 2MAR15 Other no recent HgBA1C_', 
                '(([[:digit:]]{1,2})-([[:alpha:]]{3,9})-([[:digit:]]{2,4}))|(([[:digit:]]{1,2}) ([[:alpha:]]{3,9}) ([[:digit:]]{2,4}))|(([[:digit:]]{1,2})\/([[:digit:]]{1,2})\/([[:digit:]]{2,4}))|
                 (([[:alpha:]]{2,9}) ([[:digit:]]{2,4}),([[:digit:]]{2,4}))|(([[:digit:]]{1,2})([[:alpha:]]{3,9})([[:digit:]]{2,4}))|([[:digit:]]{4,10})|(HgbA1C)|(HgBA1C)'),
               '([-?!.\=/+,?!><$*#^@%)(&]+$)|(^[-?!.0\=/+,?!><$*#@%)(0&]+)|([[:alpha:]-)(#?!$%])')),
             '^[-?!.0\=/+,?!><$*#@%]'),
            '([:*&%$;=>/`])|(\.+$)')
        ), 1, 3)
