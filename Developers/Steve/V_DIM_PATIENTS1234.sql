CREATE OR REPLACE VIEW v_dim_patients1 AS
SELECT
  p.network,
  p.patient_id,
  d.archive_number,
  ...
  CASE
    ...
    WHEN NVL(d.cell_phone, '$$N/A$$') <> b.cell_phone THEN 'CELL_PHONE'
...
LEFT JOIN patient_care_provider pcp   ON pcp.network = p.network AND pcp.patient_id = p.patient_id  AND pcp.patient_care_provider_number = 1 AND pcp.relationship_id = 1
LEFT JOIN emp_provider mep   ON mep.network = pcp.network AND mep.emp_provider_id = pcp.emp_provider_id
...
CREATE OR REPLACE TRIGGER tr_v_dim_patients1
INSTEAD OF INSERT ON v_dim_patients1 FOR EACH ROW
DECLARE
  d_from_dt DATE;
  n_archive_number NUMBER(15);
  n_network  NUMBER(5);
BEGIN

 SELECT network_key into  n_network FROM  dim_hc_networks WHERE  network = :new.network;
  ...
  ELSE
     ...
     SELECT MAX(archive_number) + 1 INTO  n_archive_number FROM  dim_patients
     WHERE  patient_id = :new.patient_id AND network = :new.network;
  ...
  VALUES
  (
     (:new.patient_id  * 50000 + n_archive_number) * 1000 +  n_network,
  --  n_network || :new.patient_id|| n_archive_number, 
